...
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct OtamatoneProfile {
    pub name: String,
    pub midi_min: u8,
    pub midi_max: u8,
    pub calibration: Vec<(u8, f64)>,
}

impl Default for OtamatoneProfile {
    fn default() -> Self {
        Self {
            name: "Standard Otamatone".into(),
            midi_min: 57,
            midi_max: 81,
            calibration: (57..=81).map(|m| (m, (m - 57) as f64 / 24.0)).collect(),
        }
    }
}
...
impl Default for AnalyzeOptions {
    fn default() -> Self {
        Self {
            ...
            merge_gap_ms: 40.0,
            transpose: 0,
            profile: OtamatoneProfile::default(),
        }
    }
}
