...
pub fn decode_mono(path: &Path) -> Result<DecodedAudio, String> {
    ...
    let probed = get_probe()
        ...
        .map_err(|error| format!("Unsupported or undecodable audio format: {error}"))?;
    let mut format = probed.format;
    let track = format
        .default_track()
        .ok_or_else(|| "Audio file contains no default audio track".to_owned())?;
    let sample_rate = track
        .codec_params
        .sample_rate
        .ok_or_else(|| "Audio sample rate is unavailable".to_owned())?;
    ...
    loop {
        let packet = match format.next_packet() {
            ...
            Err(error) => return Err(format!("Audio packet decode failed: {error}")),
        };
    ...
    if mono.is_empty() {
        return Err("Audio file contains no decodable samples".to_owned());
    }
    Ok(DecodedAudio {
        samples: mono,
        sample_rate,
...
fn append_mono(destination: &mut Vec<f32>, buffer: AudioBufferRef<'_>) {
    ...
    for frame in samples.samples().chunks(channels) {
        destination.push(frame.iter().sum::<f32>() / channels as f32);
    }
}
